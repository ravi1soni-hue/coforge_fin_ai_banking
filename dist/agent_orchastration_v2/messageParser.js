 ...
 */
export function isExplicitProductRequest(message) {
...
export function extractAmount(message) {
    ...
    const wordMatch = /([\d,]+(?:\.\d{1,2})?)\s*(gbp|pound|pounds|eur|euro|euros|usd|dollar|dollars)/i.exec(message);
    if (wordMatch) {
 ...
 */
export function extractTimeHorizon(message) {
    const patterns = [
 ...
 */
export function inferGoalTypeFromMessage(message) {
    const lower = message.toLowerCase();
    if (/\b(trip|travel|holiday|vacation|flight|hotel|paris|tokyo|dubai|rome|japan|france|italy|spain|germany|portugal|greece|usa|america|london|bangkok|bali|maldives|singapore|australia|canada|india|morocco|turkey|egypt)\b/.test(lower))
        return "TRIP";
    if (/\b(house|home|mortgage|rent|property|deposit|flat|apartment)\b/.test(lower))
        return "HOUSING";
    if (/\b(loan|borrow|lending|credit|finance)\b/.test(lower))
        return "LOAN";
    if (/\b(invest|isa|sip|stock|fund|portfolio|pension)\b/.test(lower))
        return "INVESTMENT";
    if (/\b(save|saving|savings|goal|target|pot)\b/.test(lower))
        return "SAVINGS";
