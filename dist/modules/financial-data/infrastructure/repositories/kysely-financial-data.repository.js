...
export class KyselyFinancialDataRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    ...
    async getUser(userId) {
        ...
        return new User(user.user_id, user.name, user.currency, user.country, employment, user.created_at || undefined, user.updated_at || undefined);
    }
    ...
    async getAccounts(userId) {
        const accounts = await this.db
            ...
            .selectAll()
            .execute();
        return accounts.map(account => new Account(account.account_id, account.user_id, account.type, account.bank, account.balance, account.average_monthly_balance, account.created_at || undefined, account.updated_at || undefined));
    }
    ...
    async getLoans(userId) {
        const loans = await this.db
            ...
            .selectAll()
            .execute();
        return loans.map(loan => new Loan(loan.loan_id, loan.user_id, loan.type, loan.provider, loan.emi, loan.remaining_tenure_months, loan.created_at || undefined, loan.updated_at || undefined));
    }
    ...
    async getSubscriptions(userId) {
        const subscriptions = await this.db
            ...
            .selectAll()
            .execute();
        return subscriptions.map(sub => new Subscription(sub.id, sub.user_id, sub.name, sub.amount, sub.cycle, sub.created_at || undefined, sub.updated_at || undefined));
    }
    ...
    async getInvestments(userId) {
        const investments = await this.db
            ...
            .selectAll()
            .execute();
        return investments.map(inv => new Investment(inv.id, inv.user_id, inv.type, inv.provider, inv.current_value, inv.monthly_contribution, inv.created_at || undefined, inv.updated_at || undefined));
    }
    ...
    async getTransactions(userId) {
        const transactions = await this.db
            ...
            .selectAll()
            .execute();
        return transactions.map(tx => new Transaction(tx.id, tx.user_id, tx.date, tx.type, tx.category, tx.amount, tx.created_at || undefined));
    }
    async saveSavingsGoals(savingsGoals) {
    ...
    async getSavingsGoals(userId) {
        const savingsGoals = await this.db
            ...
            .selectAll()
            .execute();
        return savingsGoals.map(goal => new SavingsGoal(goal.goal_id, goal.user_id, goal.target_amount, goal.target_date, goal.current_saved, goal.status, goal.created_at || undefined, goal.updated_at || undefined));
    }
