import { BenchmarkAdapter, FxAdapter, FundPriceAdapter, } from "./marketData.adapters.js";
...
const summarizeConfidence = (score, flags) => {
...
const normalizeInvestmentKey = (value) => {
...
const resolveMappedSymbol = (investmentType, map) => {
...
const deriveObservedMonths = (dates) => {
    if (!dates.length) {
        return 0;
    }
...
export class MarketDataService {
    fundAdapter = new FundPriceAdapter();
    benchmarkAdapter = new BenchmarkAdapter();
    fxAdapter = new FxAdapter();
    async buildMarketReferenceBundle(input) {
        ...
        for (const investment of input.investments) {
            ...
            const relatedDebits = input.transactions.filter((tx) => {
                ...
                return (normalizedCategory.includes(normalizedType) ||
            ...
            const monthlyContribution = typeof investment.monthlyContribution === "number" && Number.isFinite(investment.monthlyContribution)
            ...
            const estimatedCostBasis = txCostBasis > 0
                ...
                : observedMonths > 0 && monthlyContribution !== undefined
                    ...
                    : undefined;
            const estimatedProfitOrLoss = estimatedCostBasis !== undefined ? investment.currentValue - estimatedCostBasis : undefined;
            const estimatedReturnPct = estimatedCostBasis !== undefined && estimatedCostBasis > 0 && estimatedProfitOrLoss !== undefined
        ...
        const estimatedProfitOrLoss = hasCostBasis
            ...
            : undefined;
        const estimatedReturnPct = hasCostBasis && portfolioCostBasis > 0 && estimatedProfitOrLoss !== undefined
            ? (estimatedProfitOrLoss / portfolioCostBasis) * 100
            : undefined;
        const portfolioConfidenceScore = confidenceWeightSum > 0 ? confidenceScoreSum / confidenceWeightSum : 0;
