...
export const buildDeterministicSnapshot = (state) => {
    ...
    const currency = (typeof knownFacts.currency === "string" && knownFacts.currency) ||
        ...
        ? {
            ...
            confidence: {
                ...
                flags: Array.isArray(confidence?.flags)
                    ? confidence.flags.filter((flag) => typeof flag === "string")
                    : undefined,
            },
    ...
    const totalInvestmentValue = investments.length > 0
        ? investments.reduce((sum, inv) => sum + inv.currentValue, 0)
        : parseNumeric(financeInvestments?.totalCurrentValue);
    const monthlyInvestmentContribution = investments.reduce((sum, inv) => sum + (inv.monthlyContribution ?? 0), 0);
    return {
...
export const tryBuildDeterministicAnswer = (questionInput, snapshot) => {
    ...
    if (asksInvestment(question) && snapshot.totalInvestmentValue !== undefined) {
        ...
        if (asksProfit(question) && snapshot.investmentPerformance?.isComputable) {
            ...
            const confidenceSummary = confidenceScore !== undefined
                ? `${confidenceLabel} confidence (${confidenceScore.toFixed(2)})`
                : `${confidenceLabel} confidence`;
            if (pnl !== undefined) {
                const returnText = retPct !== undefined ? ` (${retPct.toFixed(2)}% return)` : "";
        ...
        if (asksProfit(question) && !snapshot.hasInvestmentCostBasis) {
            const contributionText = snapshot.monthlyInvestmentContribution !== undefined
...
export const validateAssistantAnswer = (questionInput, answer, snapshot) => {
    const question = questionInput.toLowerCase();
