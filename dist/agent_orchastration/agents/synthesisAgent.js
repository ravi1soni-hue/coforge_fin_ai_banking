...
const isAffordabilityQuestion = (state) => {
    ...
    const knownQueryType = typeof state.knownFacts?.queryType === "string"
        ...
        : "";
    const intentAction = typeof state.intent?.action === "string"
        ? state.intent.action.toLowerCase()
        : "";
    const reasoningQueryType = typeof state.reasoning?.queryType === "string"
        ? String(state.reasoning.queryType).toLowerCase()
        : "";
    return (knownQueryType === "affordability" ||
...
const buildAffordabilityReasoningAnswer = (state) => {
    ...
    const financeData = state.financeData && typeof state.financeData === "object"
        ? state.financeData
        : {};
    const cashflow = financeData.cashflow_summary && typeof financeData.cashflow_summary === "object"
        ? financeData.cashflow_summary
        : {};
    const reasoning = state.reasoning && typeof state.reasoning === "object"
        ? state.reasoning
        : {};
    const researchData = state.researchData && typeof state.researchData === "object"
        ? state.researchData
        : {};
    const costsData = researchData.costs && typeof researchData.costs === "object"
        ? researchData.costs
        : {};
    const currency = normalizeCurrency(cashflow.currency ??
        researchData.costs?.currency ??
    ...
    const netMonthlySavings = parseNumeric(cashflow.netMonthlySavings) ??
    ...
    const affordableNextMonth = typeof reasoning.affordableNextMonth === "boolean"
    ...
    const alternativeTotals = alternativesRaw
        ...
        const value = parseNumeric(costs.total);
    ...
    const hasNegativeCashflow = (typeof netMonthlySavings === "number" && netMonthlySavings < 0) ||
        (typeof monthlyIncome === "number" &&
            typeof monthlyExpenses === "number" &&
            monthlyExpenses > monthlyIncome);
    const hasDeterministicCoverage = isPositiveNumber(estimatedCost) &&
        typeof projectedNextMonthSavings === "number";
    const deterministicallyAffordableNextMonth = hasDeterministicCoverage && projectedNextMonthSavings >= estimatedCost;
    const goalType = typeof state.knownFacts?.goalType === "string" ? state.knownFacts.goalType.toLowerCase() : "";
    const purchaseLabel = (() => {
