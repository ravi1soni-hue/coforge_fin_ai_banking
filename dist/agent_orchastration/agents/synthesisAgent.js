...
export const synthesisAgent = async (state, config) => {
    ...
    if (isConfirmedAction) {
        ...
        const homeCurrency = (kf.profileCurrency ?? kf.currency ?? "GBP");
        const tripCurrency = (kf.targetCurrency ?? homeCurrency);
        // Pre-compute instalment scenarios so the LLM formats numbers, not calculates them
        const instalmentLines = [];
        if (targetAmt > 0) {
        ...
        const isInstalmentSim = state.intent?.action === "installment_simulation" ||
            /instalment|installment|split|spreading|replenish|numbers|payment.plan|months/i.test(confirmedAction);
        const effectiveTask = isInstalmentSim && targetAmt > 0
            ...
            : confirmedAction;
        const continuationAnswer = await llm.generateText(`You are a personal banking AI assistant. The user confirmed an action — execute it now.
...
7. Do NOT end with another offer or question.`);
        ...
        return {
            ...
            knownFacts: { ...kf, _pendingOffer: null }, // clear offer after fulfillment
        };
    }
    // ─────────────────────────────────────────────────────────────────────────
    const reasoningData = state.reasoning;
    ...
    const mainHomeCurrency = (() => {
        const kf = state.knownFacts ?? {};
        return (kf.profileCurrency ?? kf.currency ?? "GBP");
    })();
    const mainTripCurrency = (() => {
        const kf = state.knownFacts ?? {};
        const home = (kf.profileCurrency ?? kf.currency ?? "GBP");
        return (kf.targetCurrency ?? home);
    })();
...
KEY METRICS:
${keyMetrics.map((m) => `- ${m.label}: ${m.value}`).join("\n") || "See pre-computed analysis above."}
...
`);
    ...
    const newOffer = answer.match(/(?:want me to|shall i|would you like me to|let me|i can show you?)\s+([^.?!\n]{5,180})/i);
