...
const cs = (isoCode: string): string =>
  isoCode === "GBP" ? "£" : isoCode === "EUR" ? "€" : isoCode === "USD" ? "$" : `${isoCode} `;
...
const goalLabel = (goal: FinancialGoalContext): string => {
  const dest = goal.metadata?.destination as string | undefined;
  const item = goal.metadata?.item as string | undefined;
...
export function computeAffordabilityVerdict(
  ...
  if (cost <= 0 || availableSavings <= 0) return "CANNOT_AFFORD";

  const remaining = availableSavings - cost;
  if (remaining < 0) return "CANNOT_AFFORD";
  ...
  if (remaining < emergencyBuffer) return "RISKY";
  return "COMFORTABLE";
}
...
// ─── 3. Intent classifier (minimal LLM call) ──────────────────────────────────
...
{ "intent": string, "domain": string, "reasoning": string }`);
  ...
  const intent = (result.intent ?? "INFO_ONLY").trim() as IntentType;
  const domain = (result.domain ?? "GENERAL_BANKING").trim() as DomainType;
  const reasoning = (result.reasoning ?? "NONE").trim() as ReasoningLevel;

  // Validate — fall back safely if LLM returns unexpected value
  const validIntents: IntentType[] = ["INFO_ONLY", "AFFORDABILITY_CHECK", "PLANNING", "COMPARISON", "ACTION_SUGGESTION"];
  const validDomains: DomainType[] = ["TRAVEL", "CONSUMER_PURCHASE", "HOUSING", "LOAN", "SAVINGS", "INVESTMENT", "SUBSCRIPTION", "LIFESTYLE", "GENERAL_BANKING"];
  const validReasoning: ReasoningLevel[] = ["NONE", "LIGHT", "HEAVY"];
...
export async function generateCostQuestion(
  ...
  const context = goal?.metadata?.destination
    ? ` to ${goal.metadata.destination as string}`
    : goal?.metadata?.item
      ? ` for ${goal.metadata.item as string}`
      : "";
...
export async function generateTimeHorizonQuestion(
  llm: LlmClient,
  originalQuestion: string,
): Promise<string> {
  return llm.generateText(
...
export async function generateAffordabilityAnswer(
  ...
  const verdictLabel =
    verdict === "COMFORTABLE"   ? "✅ COMFORTABLE — can afford without risk"
...
export async function generatePlanningAnswer(
  llm: LlmClient,
  vectorQuery: VectorQueryService,
  userId: string,
  question: string,
  profile: UserProfile,
  goal: FinancialGoalContext | undefined,
  history: ConversationTurn[],
): Promise<string> {
  const context = await vectorQuery.getContext(
    userId,
    `savings plan investment goal: ${question}`,
    { topK: 6 },
  );
  ...
  const monthsToGoal =
    goal?.cost && netMonthlySurplus && netMonthlySurplus > 0
...
export async function generateInfoAnswer(
  llm: LlmClient,
  vectorQuery: VectorQueryService,
  userId: string,
  question: string,
...
export async function generateComparisonAnswer(
  ...
  const context = await vectorQuery.getContext(
    userId,
    `comparison financial options: ${question}`,
    { topK: 6 },
  );
