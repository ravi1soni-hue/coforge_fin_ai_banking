 ...
 */
export const syncUserFinancialProfiles = async (db: Kysely<any>): Promise<void> => {
  try {
    ...
    const txStats = deriveTransactionStats(parsed.transactions as Record<string, unknown>[] | undefined);
    ...
    if (existing) {
      const changed =
        existing.monthly_income !== monthlyIncome ||
      ...
      if (changed) {
        ...
        console.log(
          `   Old: Income=${existing.monthly_income}, Expenses=${existing.monthly_expenses}, Savings=${existing.net_monthly_savings}`
        );
