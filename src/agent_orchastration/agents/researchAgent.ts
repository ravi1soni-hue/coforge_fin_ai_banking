...
const sanitizeAffordabilityResult = (
  ...
  if (!trustedCost) {
    assumptions.push(
      "No reliable user-provided or web-searched numeric goal cost was available; affordability verdict should remain conditional until target amount is provided."
    );
  } else if (webSearchCost && !knownFactsBudget) {
    assumptions.push(
      `Cost sourced from live DuckDuckGo Instant Answer search (query: "${searchResult?.query ?? ""}", confidence: ${searchResult?.confidence ?? "partial"}).`
    );
  }

  const costSource: "user_input" | "web_search" | "missing" = knownFactsBudget
    ? "user_input"
    : webSearchCost
    ? "web_search"
    : "missing";
