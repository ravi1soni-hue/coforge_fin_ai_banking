...
const buildCatalogBlock = (): string =>
  PRODUCT_CATALOG.map(
    (p) =>
      `[${p.code}] ${p.name} (${p.category}, risk: ${p.riskLevel})\n` +
      `  Tagline: ${p.tagline}\n` +
...
export const productRecommendationAgent = async (
  ...
  const cashflow = (
    (state.financeData as Record<string, unknown> | undefined)?.cashflow_summary ?? {}
  ) as Record<string, unknown>;
  const netMonthlySavings =
    typeof cashflow.netMonthlySavings === "number" ? cashflow.netMonthlySavings : undefined;
  const monthlyIncome =
    typeof cashflow.monthlyIncome === "number" ? cashflow.monthlyIncome : undefined;
  const goalType =
    typeof state.knownFacts?.goalType === "string" ? state.knownFacts.goalType : "unknown";
  const queryType =
    typeof state.knownFacts?.queryType === "string" ? state.knownFacts.queryType : "unknown";
  const intentAction =
    typeof state.intent?.action === "string" ? state.intent.action : "unknown";
  const reasoning = (state.reasoning ?? {}) as Record<string, unknown>;
  const affordable = reasoning.affordable;
  const affordableNextMonth = reasoning.affordableNextMonth;
  const shortfall =
    typeof reasoning.shortfallAmount === "number" && reasoning.shortfallAmount > 0
