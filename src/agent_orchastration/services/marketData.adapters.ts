...
const normalizeConfidence = (score: number): ConfidenceLabel => {
  if (score >= 0.8) {
    return "high";
  }
  if (score >= 0.55) {
...
const conf = (score: number, flags: string[]): ConfidenceMeta => {
...
const inferCurrencyFromSymbol = (symbol: string): string | undefined => {
...
const parseStooqCsv = (csv: string): Record<string, string> | undefined => {
...
export class FundPriceAdapter {
  private readonly baseUrl = ENV.MARKET_DATA_STOOQ_BASE_URL;
  private readonly timeoutMs = ENV.MARKET_DATA_TIMEOUT_MS;

  async getLatest(symbol: string): Promise<QuoteResult> {
    if (!symbol) {
      return {
        symbol,
        source: "stooq",
...
export class FxAdapter {
  private readonly baseUrl = ENV.MARKET_DATA_FX_BASE_URL;
  private readonly timeoutMs = ENV.MARKET_DATA_TIMEOUT_MS;
