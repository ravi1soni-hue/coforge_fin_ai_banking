...
function enrichPriceQuery(query: string): string {
  ...
  if (hasPriceWord && hasRegion) return query;
  if (hasPriceWord) return `${query} UK`;
...
async function researchPrice(
  ...
  console.log(
    `[ResearchAgent:Price] 📄 Web context length: ${webContext.length}`
  );
...
Extract the explicit price strictly from the web data.`,
  ...
  if (
    ...
    return {
      ...
      source: "web_search",
      confidence: (parsed.confidence as PriceInfo["confidence"]) ?? "medium",
...
export async function runResearchAgent(
  ...
  const userStatedPriceInfo: PriceInfo | null =
    plan.userStatedPrice && plan.userStatedPrice > 0
      ? {
  ...
  const result: ResearchResult = { priceInfo, fxInfo, newsInfo };
