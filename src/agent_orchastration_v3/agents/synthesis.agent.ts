...
function buildDataContext(state: FinancialState): string {
  ...
  if (state.treasuryAnalysis) {
    ...
    parts.push(
      ...
      `- Urgent supplier amount: ${t.urgentSupplierTotal.toLocaleString("en-GB")} ${t.currency}`,
      `- Deferable supplier amount: ${t.deferableSupplierTotal.toLocaleString("en-GB")} ${t.currency}`,
      `- Projected low balance: ${t.projectedLowBalance.toLocaleString("en-GB")} ${t.currency}`,
      `- Projected low (full release): ${t.projectedLowBalanceIfFullRelease.toLocaleString("en-GB")} ${t.currency}`,
      `- Projected low (split): ${t.projectedLowBalanceIfSplit.toLocaleString("en-GB")} ${t.currency}`,
      ...
      `- Min inflow needed for midweek release: ${t.minInflowForMidweekRelease.toLocaleString("en-GB")} ${t.currency}`,
      `- Release-condition hit rate (10 weeks): ${(t.releaseConditionHitRate10Weeks * 100).toFixed(0)}%`,
