...
export async function runSupervisorAgent(
  ...
  const lastTurns: { role: string; content: string }[] = [];
  ...
  if (lastUser.length > 0) lastTurns.push({ role: "user", content: lastUser[0].content });
  if (lastAssistant.length > 0) lastTurns.push({ role: "assistant", content: lastAssistant[0].content });
  if (lastUser.length > 1) lastTurns.push({ role: "user", content: lastUser[1].content });
  ...
  const plan: AgentPlan = {
    ...
    intent:             (parsed.intent as any)         || "other",
